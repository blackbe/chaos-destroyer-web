...
import { checkAndGenerateReminders, showRemindersMenu } from './reminders';
import { store } from './store';
import { runPayoffPlanner } from './payoff';
...
async function mainMenu() {
  checkAndGenerateReminders(); // Auto-generate on launch

  console.clear();
  console.log(chalk.cyan.bold('\n┌─────────────────────────────────────┐'));
  console.log(chalk.cyan.bold('│  ✨ CHAOS DESTROYER - MAIN MENU     │'));
  console.log(chalk.cyan.bold('└─────────────────────────────────────┘\n'));
  ...
  switch (action) {
    ...
    case 'Reminders (View/Manage)':
      await showRemindersMenu();
      break;
    case 'Payoff Planner (Scenarios)':
      await runPayoffPlanner();
      break;
...
async function debtsMenu() {
  console.clear();
  console.log(chalk.cyan.bold('💳 MANAGE DEBTS\n'));

  const debts = (store.getDebts() || []).filter(d => d.type === 'debt').sort((a, b) => a.payoffPriority - b.payoffPriority);

  if (debts.length === 0) {
    console.log(chalk.yellow('No debts found.\n'));
  } else {
    ...
    debts.forEach((d, idx) => {
      const color = d.payoffPriority === 1 ? chalk.red.bold : chalk.white;
      debtTable.push([
        color(`${idx + 1}`),
        color(d.name),
  ...
  const { action } = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        ...(debts.length > 1 ? [new inquirer.Separator(), 'Reorder Debts (Priority)'] : []),
        new inquirer.Separator(),
        'Add New Debt',
        'Back'
      ]
  ...
  if (action === 'Reorder Debts (Priority)') {
    ...
    await addDebtFlow();
    console.log(chalk.green('✅ Debt added!\n'));
    await pressKey();
    return debtsMenu();
    ...
    if (confirm) {
      ...
      console.log(chalk.green('✅ Debt deleted!\n'));
    }
    await pressKey();
    return debtsMenu();
  }
...
async function reorderDebtsFlow() {
  console.clear();
  console.log(chalk.cyan.bold('🔄 REORDER DEBTS BY PRIORITY\n'));
  ...
  const debt = store.getDebt(debtId);
  if (!debt) return;
  ...
  const newPriority = await inquirer.prompt([
    {
      ...
      validate: (val) => {
        if (val < 1 || val > debts.length) return `Must be between 1 and ${debts.length}`;
        return true;
      }
    }
  ]);
  ...
  // Reassign priorities
  const allDebts = store.getDebts().filter(d => d.type === 'debt');
  ...
  if (targetPriority < currentPriority) {
    ...
    allDebts.forEach(d => {
      if (d.id === debtId) {
        store.updateDebt(d.id, { payoffPriority: targetPriority });
      } else if (d.payoffPriority >= targetPriority && d.payoffPriority < currentPriority) {
        store.updateDebt(d.id, { payoffPriority: d.payoffPriority + 1 });
      }
    });
  } else {
  ...
  console.log(chalk.green(`✅ Priority updated! ${debt.name} is now priority ${targetPriority}\n`));
  await pressKey();
}
...
async function addDebtFlow() {
  ...
  const errors = store.validateDebt(answers);
  if (errors.length > 0) {
...
async function editDebtFlow(debtId: string) {
  const debt = store.getDebt(debtId);
  if (!debt) return;

  console.clear();
  console.log(chalk.cyan(`Editing: ${debt.name}\n`));
  console.log(`Balance: ${formatCurrency(debt.balance)}`);
  console.log(`APR: ${debt.apr}%`);
  console.log(`Min Payment: ${formatCurrency(debt.minimumPayment)}\n`);
  ...
  if (field === 'Balance') updates.balance = value;
  if (field === 'APR') updates.apr = value;
  if (field === 'Min Payment') updates.minimumPayment = value;
  if (field === 'Due Day') updates.dueDate = value;
  if (field === 'Priority') updates.payoffPriority = value;
  if (field === 'Notes') updates.notes = value;

  store.updateDebt(debtId, updates);
...
async function billsMenu() {
  console.clear();
  console.log(chalk.cyan.bold('📄 MANAGE BILLS\n'));
  ...
  if (action === 'Add New Bill') {
    await addBillFlow();
    console.log(chalk.green('✅ Bill added!\n'));
    await pressKey();
    return billsMenu();
  } else if (action.startsWith('edit-')) {
    ...
    if (confirm) {
      ...
      console.log(chalk.green('✅ Bill deleted!\n'));
    }
    await pressKey();
    return billsMenu();
  }
...
async function editBillFlow(billId: string) {
  const bill = store.getBill(billId);
  if (!bill) return;

  console.clear();
  console.log(chalk.cyan(`Editing: ${bill.name}\n`));
  console.log(`Amount: ${formatCurrency(bill.amount)}`);
  ...
  const updates: any = {};
  if (field === 'Name') updates.name = value;
  if (field === 'Amount') updates.amount = value;
  if (field === 'Due Day') updates.dueDate = value;
  if (field === 'Frequency') {
  ...
  store.updateBill(billId, updates);
  console.log(chalk.green('✅ Updated!\n'));
  await pressKey();
}
...
async function incomeMenu() {
  console.clear();
  console.log(chalk.cyan.bold('💵 MANAGE INCOME\n'));

  const incomeList = store.getIncome() || [];

  if (incomeList.length === 0) {
    console.log(chalk.yellow('No income sources found.\n'));
  } else {
    const table = require('cli-table3');
  ...
  if (action === 'Add Income Source') {
    ...
    console.log(chalk.green('✅ Income source added!\n'));
    await pressKey();
    return incomeMenu();
  } else if (action.startsWith('edit-')) {
    const incomeId = action.replace('edit-', '');
    const inc = store.getIncome().find(i => i.id === incomeId);
    if (inc) {
      const { field } = await inquirer.prompt([
        {
          type: 'list',
      ...
      if (field !== 'Back') {
        ...
        const updates: any = {};
        if (field === 'Name') updates.source = value;
        if (field === 'Amount') updates.amount = value;
        if (field === 'Frequency') updates.frequency = value;
        
        store.updateIncome(incomeId, updates);
        console.log(chalk.green('✅ Updated!\n'));
      }
    }
    await pressKey();
    ...
    if (inc) {
      ...
      if (confirm) {
        store.deleteIncome(incomeId);
        console.log(chalk.green('✅ Income source deleted!\n'));
      }
    }
    await pressKey();
    return incomeMenu();
...
async function recordPayment() {
  console.clear();
  console.log(chalk.cyan.bold('💳 RECORD PAYMENT\n'));

  const debts = (store.getDebts() || []).filter(d => d.type === 'debt' && d.balance > 0);
  
  if (debts.length === 0) {
    console.log(chalk.yellow('No debts to pay.\n'));
    await pressKey();
    return;
  }
  ...
  const { debtId } = await inquirer.prompt([
    {
      ...
      choices: [
        ...debts.map(d => ({
          name: `${d.name} (balance: ${formatCurrency(d.balance)})`,
          value: d.id
        })),
        new inquirer.Separator(),
  ...
  if (amount > 0) {
    ...
    if (debt) {
      console.log(chalk.cyan(`   New balance: ${formatCurrency(debt.balance)}\n`));
    }
  }
...
async function goalsMenu() {
  console.clear();
  console.log(chalk.cyan.bold('🎯 GOALS\n'));

  const goals = store.getGoals() || [];

  if (goals.length === 0) {
    console.log(chalk.yellow('No goals yet.\n'));
  } else {
    const table = require('cli-table3');
  ...
  if (action === 'Add Goal') {
    ...
    console.log(chalk.green('✅ Goal added!\n'));
    await pressKey();
    return goalsMenu();
  } else if (action.startsWith('edit-')) {
    ...
    if (goal) {
      ...
      if (field !== 'Back') {
        ...
        const updates: any = {};
        if (field === 'Name') updates.name = value;
        if (field === 'Progress') updates.progressPercent = value;
        if (field === 'Due Date') updates.dueDate = value;
        
        store.updateGoal(goalId, updates);
        console.log(chalk.green('✅ Updated!\n'));
      }
    }
    await pressKey();
    ...
    if (goal) {
      ...
      if (confirm) {
        ...
        console.log(chalk.green('✅ Goal deleted!\n'));
      }
    }
    await pressKey();
    return goalsMenu();
...
async function settingsMenu() {
  ...
  switch (action) {
    ...
    case 'View Data Directory':
      console.log(chalk.dim(`Data stored in: ${process.cwd()}/data/\n`));
      await pressKey();
      break;
  }
...
async function editDebtDates() {
  ...
  store.updateDebt(debtId, { dueDate: newDate });
  console.log(chalk.green('✅ Due date updated!\n'));
  await pressKey();
}
...
async function editBillDates() {
  ...
  store.updateBill(billId, { dueDate: newDate });
  console.log(chalk.green('✅ Due date updated!\n'));
  await pressKey();
}
...
async function showBadgesMenu() {
  ...
  if (earnedBadges.length > 0) {
    ...
    earnedBadges.forEach(badge => {
      console.log(chalk.magenta(`  ${badge.icon} ${badge.name} - ${badge.points} points`));
      if (badge.unlockedDate) {
        console.log(chalk.dim(`     Unlocked: ${badge.unlockedDate.split('T')[0]}`));
      }
    });
    console.log();
  } else {
    console.log(chalk.yellow('No badges earned yet. Keep working!\n'));
  }
  ...
  DEFAULT_BADGES.forEach(badge => {
    ...
    if (earned) {
      console.log(chalk.green(`  ${badge.icon} ${badge.name} ${chalk.dim('[EARNED]')}`));
    } else {
      const progress = getBadgeProgress(badge.id);
      if (progress) {
        const bar = drawVizBar((progress.progress / progress.required) * 100, 10);
        console.log(chalk.yellow(`  ${badge.icon} ${badge.name} - ${progress.progress}/${progress.required}`));
        console.log(chalk.yellow(`     ${bar}`));
      } else {
        console.log(chalk.dim(`  ${badge.icon} ${badge.name}`));
      }
    }
  });
  ...
  // Display streaks
  console.log(chalk.cyan.bold('\n🔥 STREAKS:\n'));
  console.log(chalk.yellow(`  Current On-Time Streak: ${gam.streaks.onTimePay} days`));
  console.log(chalk.yellow(`  Best Streak: ${gam.streaks.bestOnTimeStreak} days`));
  console.log(chalk.magenta(`  Total Chaos Points: ${gam.chaoPoints}`));
  console.log();
...
async function showChallengesMenu() {
  ...
  if (!challenge) {
    console.log(chalk.green.bold('✅ Challenge completed today! Come back tomorrow for a new one.\n'));
    console.log(chalk.dim(`Points earned this month: ${gam.chaoPoints}`));
  } else {
    console.log(chalk.cyan(`Today's Challenge: ${chalk.bold(challenge.title)}\n`));
    console.log(chalk.white(`${challenge.description}\n`));
    console.log(chalk.magenta(`Reward: +${challenge.points} Chaos Points\n`));
    ...
    if (complete) {
      ...
      console.log();
      console.log(chalk.green.bold(`✅ Challenge Complete! +${points} Chaos Points`));
      console.log(chalk.magenta(`Total Points: ${gam.chaoPoints + points}`));
      console.log();
    }
  }
...
// === UTILITIES ===
function drawVizBar(percent: number, width: number = 10): string {

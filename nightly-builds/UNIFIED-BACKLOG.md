...
[x] 90. [CHAOS-DESTROYER] Weekend social calendar planner — Suggest activities, track West Coast Swing events
[x] 91. [HUGBACK] Share location with trusted friend feature — From Trello Phase 4 #85
[x] 92. [CHAOS-DESTROYER] Therapy homework organizer — Track assignments, practice logs, insights. BUILD A FULL WORKING FEATURE: Create a React component with a clean UI that lets Ben track therapy assignments (homework from his Tuesday sessions), log when he practices skills between appointments, and capture insights gained. Include: (1) A dashboard showing upcoming assignments and due dates, (2) Ability to add new therapy sessions with date/assignment, (3) Practice log entries with description, mood rating (1-5), and duration, (4) Insight capture with tags (e.g., 'ADHD', 'self-confidence', 'childhood trauma'), (5) Progress visualization showing consistency over time, (6) Export to JSON for therapist sharing. Make it visually appealing — not bare console.log classes. Integrate into the existing chaos-destroyer app structure with proper styling.
